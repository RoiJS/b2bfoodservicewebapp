(function() {
    ...
    angular
        .module("appMain")
        .controller("AppUpdateItemQuantityDialogController", AppUpdateItemQuantityDialogController);
    
    AppUpdateItemQuantityDialogController.$inject = ["$scope"];

    function AppUpdateItemQuantityDialogController($scope) {
        ...
        vm.selectedQuantity = 0;            // Scope variable used on <input ng-model="selectedQuantity"> on this controllers' view template (app-update-item-quantity-dialog-template.html).
        ...
        function activate() {
            ...
            vm.selectedQuantity = vm.currentSelectedTotal;
        }
        ...
        function addQuantity() {
            if (vm.selectedQuantity <= vm.itemDetails.quantity) {
        ...
        function closeDialog(isConfirm) {
            ...
            if (isConfirm) {
                alert(`${(vm.selectedQuantity - vm.currentSelectedTotal)} new quantities for this item has been added your cart.`);

                vm.closeThisDialog(new Order({
                    categoryId: vm.itemDetails.categoryId,
                    itemId: vm.itemDetails.itemId,
                    quantity: (vm.selectedQuantity - vm.currentSelectedTotal),
                    amount: ((vm.selectedQuantity - vm.currentSelectedTotal) * vm.itemDetails.current_price)
                }));
            } else {
                vm.closeThisDialog();
            }
